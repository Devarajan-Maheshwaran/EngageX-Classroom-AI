"""
routers/signals.py — Phase 6
Signal ingestion endpoints.
Phase 6 note: browser now sends NLP-enriched text signals
(sentiment, intent, intent_scores, engagement_score).
"""
...
def ingest_text_signal(body: TextSignalBody):
    ...
    try:
        ...
        logger.info(
            f"[Signal/text] {signal_type} student={body.student_id[:8]} "
            f"intent={body.intent} sentiment={body.sentiment} score={body.engagement_score}"
        )
...
async def ingest_audio_signal(
    ...
    logger.info(
        f"[Signal/audio] received student={student_id[:8]} file={audio.filename} size={size}B"
    )
